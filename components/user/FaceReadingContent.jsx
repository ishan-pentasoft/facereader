...
const FaceReadingContent = () => {
  const [faceReading, setFaceReading] = useState(null);
  const [isLoading, setIsLoading] = useState(true);

  const fetchFaceReading = async () => {
    try {
      setIsLoading(true);
      const response = await fetch("/api/user/pages/face-reading", {
        method: "GET",
      });
      if (!response.ok) {
      ...
      setFaceReading(data.success ? data.faceReading : null);
    } catch (error) {
        ...
        {isLoading ? (
          ...
          >
            ...
            <motion.div variants={imageVariants}>
              <Image
                src={faceReading.image}
                alt={faceReading.name}
                width={500}
                height={500}
                draggable={false}
            ...
            <motion.div
              ...
              variants={itemVariants}
              dangerouslySetInnerHTML={{ __html: faceReading.description }}
            />
          </motion.section>
        ) : (
          <motion.div
          ...
          >
            <p className="text-muted-foreground">
              Failed to load face reading information.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
...
export default FaceReadingContent;
