...
const VastuContent = () => {
  const [vastu, setVastu] = useState(null);
  const [isLoading, setIsLoading] = useState(true);

  const fetchVastu = async () => {
    try {
      setIsLoading(true);
      const response = await fetch("/api/user/pages/vastu", {
        method: "GET",
      });
      if (!response.ok) {
      ...
      setVastu(data.success ? data.vastu : null);
    } catch (error) {
        ...
        {isLoading ? (
          ...
          >
            ...
            <motion.div
              ...
              variants={itemVariants}
              dangerouslySetInnerHTML={{ __html: vastu.description }}
            />
          </motion.section>
        ) : (
          <motion.div
          ...
          >
            <p className="text-muted-foreground">
              Failed to load vastu information.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
