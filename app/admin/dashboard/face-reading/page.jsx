...
export default function AstrologyPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/admin/pages/face-reading", {
          credentials: "include",
        });
        if (!res.ok) throw new Error("Failed to load face reading data");
        ...
        toast.error("Could not load face reading data");
      } finally {
  ...
  async function onSubmit(values) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/pages/face-reading", {
        ...
        credentials: "include",
        body: JSON.stringify(faceReadingData),
      });

      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err?.error || "Failed to save face reading data");
      }
      ...
      toast.success("Face reading information saved successfully");
    } catch (e) {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl p-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-xl">Face Reading Page</CardTitle>
          <CardDescription>
            Manage the face reading page content displayed on your website.
          </CardDescription>
        </CardHeader>
