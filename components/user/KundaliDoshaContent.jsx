...
const KundaliDoshaContent = () => {
  const [kundaliDosha, setKundaliDosha] = useState(null);
  const [isLoading, setIsLoading] = useState(true);

  const fetchKundaliDosha = async () => {
    try {
      setIsLoading(true);
      const response = await fetch("/api/user/pages/kundali-dosha", {
        method: "GET",
      });
      if (!response.ok) {
      ...
      setKundaliDosha(data.success ? data.kundaliDosha : null);
    } catch (error) {
        ...
        {isLoading ? (
          ...
          >
            ...
            <motion.div variants={imageVariants}>
              <Image
                src={kundaliDosha.image}
                alt={kundaliDosha.name}
                width={500}
                height={500}
                draggable={false}
            ...
            <motion.div
              ...
              variants={itemVariants}
              dangerouslySetInnerHTML={{ __html: kundaliDosha.description }}
            />
          </motion.section>
        ) : (
          <motion.div
          ...
          >
            <p className="text-muted-foreground">
              Failed to load Kundali Dosha information.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
...
export default KundaliDoshaContent;
