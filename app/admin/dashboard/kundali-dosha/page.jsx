...
export default function KundaliDoshaPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/admin/pages/kundali-dosha", {
          credentials: "include",
        });
        if (!res.ok) throw new Error("Failed to load kundali-dosha data");
        ...
        toast.error("Could not load kundali dosha data");
      } finally {
  ...
  async function onSubmit(values) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/pages/kundali-dosha", {
        ...
        credentials: "include",
        body: JSON.stringify(kundaliDoshaData),
      });

      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err?.error || "Failed to save kundali dosha data");
      }
      ...
      toast.success("Kundali Dosha information saved successfully");
    } catch (e) {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl p-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-xl">Kundali Dosha Page</CardTitle>
          <CardDescription>
            Manage the Kundali Dosha page content displayed on your website.
          </CardDescription>
        </CardHeader>
